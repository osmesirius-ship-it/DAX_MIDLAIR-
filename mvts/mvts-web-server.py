"""
MVTS Web Server
Complete web interface for Minimum Viable Thinking Server
"""
...
# Import MVTS components directly
import sys
import os
...
app = FastAPI(title="MVTS Web Server")
...
async def startup_event():
    ...
    try:
        ...
        logger.info("MVTS Web Server initialized")
    except Exception as e:
        logger.error(f"Failed to initialize MVTS: {e}")

@app.get("/", response_class=HTMLResponse)
async def home():
    """Main MVTS web interface"""
    ...
    <script>
        ...
        async function updateStatus() {
            try {
                ...
                document.getElementById('system-status').className = `text-2xl font-bold ${status.status === 'healthy' ? 'text-green-400' : 'text-yellow-400'}`;
...
if __name__ == "__main__":
    print("Starting MVTS Web Server...")
    print("Web Interface: http://localhost:8004")
    print("API Health: http://localhost:8004/health")
