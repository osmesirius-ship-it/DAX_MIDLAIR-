"""
DAX-MVTS Integrated Server with ChatGPT LLM Backend
Combines DAX governance with MVTS cognitive loops using OpenAI ChatGPT
"""
...
# Import DAX ChatGPT core directly
dax_llm_path = os.path.join(os.path.dirname(__file__), '..', 'backend', 'dax-chatgpt-core.py')
...
app = FastAPI(title="DAX-MVTS ChatGPT Integrated Server")

# Global instances
mvts_core: Optional[MVTSCore] = None
dax_core: Optional[DAXChatGPTCore] = None
...
async def startup_event():
    """Initialize integrated DAX-MVTS server with ChatGPT"""
    global mvts_core, dax_core, autonomous_running
    
    try:
        # Initialize MVTS
        mvts_core = MVTSCore({
            "storage_path": "./dax-chatgpt-integrated-state.json",
            "auto_apply_rules": True,
            "rule_application_interval": 15
        })
        
        # Initialize DAX with ChatGPT
        try:
            dax_core = await initialize_dax()
            logger.info("DAX ChatGPT Core initialized successfully")
        except Exception as e:
            logger.warning(f"DAX ChatGPT Core initialization failed: {e}")
        ...
        logger.info("DAX-MVTS ChatGPT Integrated Server initialized")
        
    except Exception as e:
        logger.error(f"Failed to initialize server: {e}")

def autonomous_processor():
    """Background autonomous processing with real ChatGPT AI"""
...
async def process_dax(request: DAXRequest):
    ...
    if not dax_core:
        raise HTTPException(status_code=503, detail="DAX ChatGPT not available")
...
# Enhanced HTML Response with ChatGPT Integration
HTML_RESPONSE = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>DAX-MVTS ChatGPT Integrated Server</title>
    ...
    <style>
        ...
        .chatgpt-available { border-left: 4px solid #10b981; }
        .chatgpt-unavailable { border-left: 4px solid #ef4444; }
...
<body class="bg-gray-900 text-white">
    ...
    <div id="sidebar" class="sidebar fixed left-0 top-0 h-full bg-gray-800 border-r border-gray-700 overflow-y-auto">
        <div class="p-4">
            <h2 class="text-xl font-bold mb-4 text-green-400">DAX-MVTS + ChatGPT</h2>
            
            <!-- LLM Status -->
            <div id="llm-status-card" class="mb-6 p-3 bg-gray-700 rounded-lg">
                <h3 class="text-sm font-semibold mb-2">ChatGPT Backend</h3>
            ...
            <div class="space-y-2">
                ...
                <div class="text-xs space-y-1">
                    ...
                    <div>ChatGPT Status: <span id="dax-health" class="font-mono">Unknown</span></div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="main-content">
        <div class="container mx-auto px-4 py-6">
            <header class="mb-8">
                <h1 class="text-4xl font-bold text-center mb-2 bg-gradient-to-r from-green-400 to-blue-600 bg-clip-text text-transparent">
                    DAX-MVTS + ChatGPT Server
enta
               这个小
               dual
               ALSE
                </h1>
                <p class="text-center text-gray-400">Real AI Governance with ChatGPT + Cognitive Loop Processing</p>
            </header>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                ...
                <div class="bg-gray-800 rounded-lg p-4">
                    <h3 class="text-sm font-semibold text-gray-400 mb-1">ChatGPT Available</h3>
            ...
            <div class="bg-gray-800 rounded-lg p-6 mb-8">
                <h2 class="text-xl font-semibold mb-4">ChatGPT-Powered Processing</h2>
                <div class="space-y-4">
                    <div>
                        ...
                        <textarea id="input-text" rows="3" 
                                  ...
                                  placeholder="Enter your request for DAX-MVTS ChatGPT processing..."></textarea>
                    </div>
        ...
        async function updateStatus() {
            try {
                ...
                if (llmAvailable && status.dax_status?.status === 'healthy') {
                    ...
                    document.getElementById('llm-status-card').classList.add('chatgpt-available');
                    document.getElementById('llm-status-card').classList.remove('chatgpt-unavailable');
                } else {
                    llmStatus.innerHTML = '<div class="w-2 h-2 bg-red-400 rounded-full mr-2"></div><span class="text-sm">Unavailable</span>';
                    llmDetails.textContent = status.dax_status?.error || 'ChatGPT backend not configured';
                    llmAvailableMain.textContent = 'No';
                    llmAvailableMain.className = 'text-2xl font-bold text-red-400';
                    document.getElementById('llm-status-card').classList.remove('chatgpt-available');
                    document.getElementById('llm-status-card').classList.add('chatgpt-unavailable');
                }
        ...
        function displayResults(results, input) {
            ...
            if (results.dax) {
                if (results.dax.success) {
                    html += `<div class="mt-3 p-3 bg-green-900 rounded">
                        <h4 class="font-semibold text-green-300">ChatGPT Result:</h4>
                        <p class="text-sm mt-1">${results.dax.output}</p>
                    </div>`;
                } else {
                    html += `<div class="mt-3 p-3 bg-red-900 rounded">
                        <h4 class="font-semibold text-red-300">ChatGPT Error:</h4>
...
if __name__ == "__main__":
    print("Starting DAX-MVTS ChatGPT Integrated Server...")
    print("Web Interface: http://localhost:8007")
    print("Features: Real ChatGPT LLM backend, DAX 13-layer governance, MVTS cognitive loops")
    print("Note: Requires OPENAI_API_KEY environment variable for full functionality")
